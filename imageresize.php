 ...
 *      aimgresize.php
 *      
 *      Copyright 2013 Almas <almas@dusal.net>
 *      
...
class plgContentImageResize extends JPlugin {
    ...
    function onContentPrepare($context, &$article, &$params, $limitstart = 0) {
         $this->plgImageResize($article, $params, $limitstart);
        return true;
    }
    ...
    function resizeImageToThumb($img, $w, $h, $newfilename) {
     ...
     switch ($imgInfo[2]) {
      ...
      case 3: 	if($this->getExtension($newfilename)=='png') {
    ...
    function plgImageResize(&$row, &$params, $page)
    {
        ...
        $pattern = '/<img.([^>]*)src="[^A-Za-z0-9]*images\\/([^"]*)".([^>]*)>/';
        ...
        {
            $replacements[] =  $this->getCache($matches[2][$i], $matches[1][$i].''.$matches[3][$i]);	
    ...
    function getCache($imagePath, $restofTag)
    {
        ...
        $imageDir = "images/".  $this->getDir($imagePath);
        $imageCacheDir = "cache/images/" .  $this->getDir($imagePath);


        if(strstr($restofTag, 'class="transparent"') ||  $this->getExtension($fileName)=='jpg') { $fileNameCache = $width . "-" . $fileName; } else { $fileNameCache = $width . "-" . $fileName . '.jpg'; }
        ...
        if(!file_exists($cache)) {
            if(!$this->resizeImageToThumb($imageDir . "/" . $fileName, $width, $height, $cache)) { $nochange == true; }
        }
